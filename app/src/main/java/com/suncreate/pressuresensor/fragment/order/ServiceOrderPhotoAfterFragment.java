...
import com.suncreate.pressuresensor.bean.user.ServicesOrderDetail;
...
import static com.suncreate.pressuresensor.fragment.order.ServiceOrderDetailFragment.ORDER_ACTION;
import static com.suncreate.pressuresensor.fragment.order.ServiceOrderDetailFragment.ORDER_VIEW;
import static com.suncreate.pressuresensor.fragment.order.ServiceOrderDetailFragment.RESULT_CODE_PHOTO_POST;
...
public class ServiceOrderPhotoAfterFragment extends BaseFragment implements View.OnClickListener {
    ...
    protected void initData() {
        ...
        if(null==mDetail.getOrderAfterTime()){
            llTitleTime.setVisibility(View.GONE);
        }else {
            tvTime.setText("服务后拍照时间：" + DatePro.formatDay(mDetail.getOrderAfterTime(), "yyyy-MM-dd HH:mm"));
        }
        if (mDetail != null) {
            ...
            if (mAction == ORDER_VIEW) {
                ...
                setImageFromNet(R.id.iv_report_photo_1, MonkeyApi.getPartImgUrl(mDetail.getAfterPhoto1()), R.drawable.logo);
                setImageFromNet(R.id.iv_report_photo_2, MonkeyApi.getPartImgUrl(mDetail.getAfterPhoto2()), R.drawable.logo);
                setImageFromNet(R.id.iv_report_photo_3, MonkeyApi.getPartImgUrl(mDetail.getAfterPhoto3()), R.drawable.logo);
            }
        } else {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.iv_report_photo_1:
                OSCPhotosActivity.showImagePrivew(getContext(), MonkeyApi.getPartImgUrl(mDetail.getAfterPhoto1()));
                break;
            case R.id.tv_take_photo_2:
                mRecyclerImages2.onLoadMoreClick1Img();
                break;
            case R.id.iv_report_photo_2:
                OSCPhotosActivity.showImagePrivew(getContext(), MonkeyApi.getPartImgUrl(mDetail.getAfterPhoto2()));
                break;
            case R.id.tv_take_photo_3:
                mRecyclerImages3.onLoadMoreClick1Img();
                break;
            case R.id.iv_report_photo_3:
                OSCPhotosActivity.showImagePrivew(getContext(), MonkeyApi.getPartImgUrl(mDetail.getAfterPhoto3()));
                break;
            case R.id.btn_submit_report:
                try {
                    ...
                    mImgAfter = new File[3];
                    String[] imgPaths1 = mRecyclerImages1.getPaths();
                    int len1 = imgPaths1.length;
                    if (len1 == 1) {
//                        mImgAfter[0] = new File(imgPaths1[0]);
                        mImgAfter[0] = getSmallImageFile(getContext(), imgPaths1[0], 300, 200, true);
                    }
                    String[] imgPaths2 = mRecyclerImages2.getPaths();
                    int len2 = imgPaths2.length;
                    if (len2 == 1) {
//                        mImgAfter[1] = new File(imgPaths2[0]);
                        mImgAfter[1] = getSmallImageFile(getContext(), imgPaths2[0], 300, 200, true);
                    }
                    String[] imgPaths3 = mRecyclerImages3.getPaths();
                    int len3 = imgPaths1.length;
                    if (len3 == 1) {
//                        mImgAfter[2] = new File(imgPaths3[0]);
                        mImgAfter[2] = getSmallImageFile(getContext(), imgPaths3[0], 300, 200, true);
                    }
                    showWaitDialog(R.string.progress_submit);
                    MonkeyApi.uploadServiceOrderImagePost(mDetail.getId(), mImgAfter, mSubmitHandler);
        ...
        public void onSuccess(int statusCode, Header[] headers, String responseString) {
            try {
                ...
                if (resultBean != null && resultBean.getCode() == 1) {
                    ...
                    getActivity().setResult(RESULT_CODE_PHOTO_POST, intent);
                    getActivity().finish();
                } else {
