...
public class TweetSelectImageAdapter extends RecyclerView.Adapter<TweetSelectImageAdapter.TweetSelectImageHolder> implements TweetPicturesPreviewerItemTouchCallback.ItemTouchHelperAdapter {
    ...
    public TweetSelectImageAdapter(Callback callback, boolean isSingle) {
        mCallback = callback;
        mIsSingleImage = isSingle;
    }
    ...
    public int getItemViewType(int position) {
        ...
        if (size >= MAX_SIZE)
            return TYPE_NONE;
        else if (position == size && !mIsSingleImage) {
            return TYPE_ADD;
        } else {
            return TYPE_NONE;
        }
    ...
    public TweetSelectImageHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.fragment_item_list_tweet_publish_selecter, parent, false);
    ...
    public int getItemCount() {
        int size = mModels.size();
        if (size == MAX_SIZE || mIsSingleImage) {
            return size;
        } else if (size == 0) {
            return 0;
        } else {
