...
public class ServiceOrdersListAdapter extends BaseListAdapter<ServicesOrder> {
    ...
    public ServiceOrdersListAdapter(Callback callback,Fragment fragment) {
        super(callback);
        mFragment=fragment;
    }
    ...
    protected void convert(ViewHolder vh, ServicesOrder item, final int position) {
        ...
        vh.setText(R.id.tv_order_total_price,
                String.format(mCallback.getContext().getResources().getString(R.string.service_order_total_price),
...
//                        itemPosition=position;
                    ...
                    case Constants.SERVICE_ORDER_TYPE.ACCOUNT:
                        ...
                        UIHelper.showSimpleBackForResult(mFragment, ServicesOrderFragment.REQUEST_CODE_SERVICE_ORDER_DETAIL, SimpleBackPage.SERVICE_ORDER_DETAIL, b);
                        break;
                    //已完成，查看订单
                    case Constants.SERVICE_ORDER_TYPE.COMPLETED:
                        break;
...
//                        itemPosition=position;
                        flag=-1;
                        DialogHelp.getConfirmDialog(mCallback.getContext(), "提示", "确定删除该订单?删除后将无法恢复，请谨慎操作!", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {
                                MonkeyApi.updateServiceOrder(String.valueOf(orderId), String.valueOf(flag), updateStatusHandler);
                            }
        ...
        public void onSuccess(int statusCode, Header[] headers, String responseString) {
            try {
                ...
                if (resultBean != null && resultBean.getCode() == 1) {
                    ...
                    if(actionPosition!=0){//不是第一列，取消后，该条记录不显示
                        if(flag==0){
